    ...
    const char *resultarray, int printResults, int printPassFail)
{
	...
	if (printResults == PRINTTEXT) {
		...
		unsigned char b;
		char *sm = (char *)"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
		    "0123456789+/";
    ...
    int printResults, int printPassFail)
{
	...
	/*
	 * hashno is included to fit the hash function pattern and to
	 * simplify testing, though it isn't used in this function.
	 */
	(void)(hashno);
	if (printResults == PRINTTEXT) {
